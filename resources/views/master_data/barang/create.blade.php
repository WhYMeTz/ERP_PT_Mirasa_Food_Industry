        ...
        <form action="{{ route('master.barang.store') }}" method="POST" style="padding: 1.5rem;">
            ...
            <div class="form-group">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.375rem;">
                    ...
                    <button type="button" class="btn btn-secondary btn-sm" data-target="barang_cd" onclick="const sel = document.getElementById('jenis_barang_id'); const opt = sel.options[sel.selectedIndex]; fetchNextCode('barang', 'barang_cd', {jenis: opt?.dataset?.cd || '', name: document.getElementById('barang_nm')?.value || ''})" style="padding: 0.2rem 0.6rem; font-size: 0.75rem;" title="Generate Ulang Nomor Urut Otomatis">
                        ↺ Auto Generate
                    </button>
                </div>
                <input type="text" id="barang_cd" name="barang_cd" value="{{ old('barang_cd', $nextBarangCode ?? '') }}" class="form-control @error('barang_cd') border-red-500 @enderror" placeholder="Contoh: BBL00G-BP1 atau BB-SK001" required>
            ...
            <div class="form-group">
                ...
                <input type="text" id="barang_nm" name="barang_nm" value="{{ old('barang_nm') }}" class="form-control @error('barang_nm') border-red-500 @enderror" placeholder="Contoh: Bumbu Jagung Bakar" oninput="const opt = document.getElementById('jenis_barang_id').options[document.getElementById('jenis_barang_id').selectedIndex]; debounceCodeFromName('barang', 'barang_nm', 'barang_cd', {jenis: opt?.dataset?.cd || ''})" required>
                @error('barang_nm')
                    <div class="form-error">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <select id="jenis_barang_id" name="jenis_barang_id" class="form-control @error('jenis_barang_id') border-red-500 @enderror" onchange="const opt = this.options[this.selectedIndex]; fetchNextCode('barang', 'barang_cd', {jenis: opt.dataset.cd || '', name: document.getElementById('barang_nm')?.value || ''})" required>
