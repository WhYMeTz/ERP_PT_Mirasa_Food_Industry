...
<form action="{{ route('master.resep.update', $bom->bom_id) }}" method="POST" id="bomForm">
    @csrf
    @method('PUT')
    ...
    <div class="card" style="margin-bottom: 1.5rem;">
        ...
        <div style="padding: 1.5rem; display: grid; grid-template-columns: repeat(auto-fit, minmax(240px, 1fr)); gap: 1.25rem;">
            <div>
                ...
                <input type="text" name="bom_no" value="{{ old('bom_no', $bom->bom_no) }}" class="form-control" style="font-family: monospace; font-weight: 700;" required>
            ...
            <div style="grid-column: span 2;">
                ...
                <input type="text" name="bom_nm" value="{{ old('bom_nm', $bom->bom_nm) }}" placeholder="Contoh: Produksi Keripik Singkong Maksi 500 (Per 100 Karton)" class="form-control" required>
            </div>
            ...
            <div>
                ...
                <select name="barang_jadi_id" id="barang_jadi_id" class="form-control" required onchange="onTargetProductChanged(this)">
                    ...
                    @foreach ($barangJadiList as $bj)
                        <option value="{{ $bj->barang_id }}" data-satuan="{{ $bj->satuanDasar?->satuan_nm }}" {{ (old('barang_jadi_id', $bom->barang_jadi_id) == $bj->barang_id) ? 'selected' : '' }}>
            ...
            <div>
                ...
                <div style="display: flex; align-items: center; gap: 0.5rem;">
                    <input type="number" step="0.0001" min="0.0001" name="batch_ukuran_qty" id="batch_ukuran_qty" value="{{ old('batch_ukuran_qty', (float) $bom->batch_ukuran_qty) }}" class="form-control" style="font-weight: 700; text-align: right;" required>
                    <span id="targetSatuanLabel" style="font-weight: 600; color: #475569; font-size: 0.875rem; white-space: nowrap;">{{ $bom->barangJadi?->satuanDasar?->satuan_nm ?? 'Unit' }}</span>
                </div>
            ...
            <div style="grid-column: 1 / -1;">
                ...
                <input type="text" name="catatan_txt" value="{{ old('catatan_txt', $bom->catatan_txt) }}" placeholder="Keterangan spesifikasi rasa, lini mesin penggorengan, standar QC, dll." class="form-control">
            </div>
        </div>
    </div>
...
@php
    $existingItemsData = $bom->details->map(function($d) {
        return [
            'barang_mentah_id' => $d->barang_mentah_id,
            'kebutuhan_qty'    => (float) $d->kebutuhan_qty,
            'satuan_nm'        => $d->barangMentah?->satuanDasar?->satuan_nm ?? 'Unit',
            'catatan_txt'      => $d->catatan_txt,
        ];
    })->values();
@endphp
